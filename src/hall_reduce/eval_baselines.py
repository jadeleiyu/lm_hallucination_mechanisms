...
from qa_datasets import MENDQADataset, FactQADataset
from util import nethook
from util.eval_metrics import compute_rewrite_quality
    ...
    for record in ds:
        ...
        if not case_result_path.exists():
            ...
            metrics = {
                ...
                "time": exec_time,
                "post": compute_rewrite_quality(edited_model, tok, record),
            }

            with torch.no_grad():
                for k, v in weights_copy.items():
                    nethook.get_parameter(model, k)[...] = v.to("cuda")
            metrics["pre"] = compute_rewrite_quality(model, tok, record)
